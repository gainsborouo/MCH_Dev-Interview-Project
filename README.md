...
2. 我們沒有限制使用大型 LLM 模型，但需要確保理解其產生的內容，我們可能會在面試中詢問相關基礎問題。
3. 如果發現範例網站、後端 API 文件無法打開的問題，請寄信至 [userwei.cs12@nycu.edu.tw](mailto:userwei.cs12@nycu.edu.tw)，我們會盡快修復。
4. 如果 GitHub Actions 顯示失敗，可以先確認 `Workflow permissions` 是否設定為 `Read and write permissions`。
